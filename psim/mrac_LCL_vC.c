...
//Entradas
  ...
  //fLCL.i2ref_k    = in[1];
  //fLCL.omega_k[4] = 0; //compensar distúrbio da rede, não usado no caso do
  fLCL.omega_k[4] = 311*sin(f_rad*t); //compensar distúrbio da rede, não usado no caso do
  fLCL.omega_k[5] = 311*cos(f_rad*t); //vc
  fLCL.omega_k[5] = 0; //vc
  fLCL.vC_k       = in[2]; //vC_k
  vcc             = in[3];
...
void calculo_acao_controle(filtro *p)
{
  ...
  p->up = kp_kd*(u_k - p->vC_k) - kp_kd*(p->u_k_1 - p->vC_k_1);

  // elementos intermediários para atualização das variáveis
  eta_k[0] = (p1+p2)*p->eta_k_1[0] -
          p1*p2*p->eta_k_2[0] +
          km*p->omega_k_2[0];

  eta_k[1] = (p1+p2)*p->eta_k_1[1] -
          p1*p2*p->eta_k_2[1] +
          km*p->omega_k_2[1];

  eta_k[2] = (p1+p2)*p->eta_k_1[2] -
          p1*p2*p->eta_k_2[2] +
          km*p->omega_k_2[2];

  eta_k[3] = (p1+p2)*p->eta_k_1[3] -
          p1*p2*p->eta_k_2[3] +
          km*p->omega_k_2[3];

  eta_k[4] = (p1+p2)*p->eta_k_1[4] -
          p1*p2*p->eta_k_2[4] +
          km*p->omega_k_2[4];

  eta_k[5] = (p1+p2)*p->eta_k_1[5] -
          p1*p2*p->eta_k_2[5] +
          km*p->omega_k_2[5];
  ...
  v_k = (p1+p2)*p->v_k_1 - p1*p2*p->v_k_2 + km*p->u_k_2;
