...
\section{Estrutura do Algoritmo Adaptativo}
    ...
    A Fig.~\ref{fig:cascata} mostra a estrutura geral do sistema de controle.
    ...
    E considerando que $y(k) = G(z) \cdot u(k)$, obtém-se:
    ...
    % Tendo em vista (\ref{eq:planta_go_delta}) e $y = G_o(z) \cdot \Delta(z)$, então:

    % \begin{equation*}
    %     y(k) = \left( G_o(z) + \Delta(z) \right) u(k)
    % \end{equation*}

    % Ou seja:

    % \begin{equation}
    %     y(k) = G_o(z) \cdot u(k) + \Delta(z) \cdot u(k)
    %     \label{eq:go_u_delta_u}
    % \end{equation}

    % De (\ref{eq:funcao_de_fs}):
    ...
    Com $y_m(k) = W_m(z) r(k)$ e $\rho^* = \frac{1}{\theta_4^*}$:
